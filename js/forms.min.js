    ...
    window.scriptSysPayment = {}; window.handlerSysPayment = {}; window.tildaForm = { versionLib: '01.001', isRecaptchaScriptInit: !1, currentFormProccessing: !1, arMessages: { 'EN': { 'success': 'Thank you! Your data has been submitted.', 'successorder': 'Thank you! Order created. Please wait. We are going to the payment...' }, 'RU': { 'success': 'Спасибо! Данные успешно отправлены.', 'successorder': 'Спасибо! Заказ оформлен. Пожалуйста, подождите. Идет переход к оплате....' } }, arValidateErrors: { 'EN': { 'email': 'Please put a correct e-mail', 'url': 'Please put a correct URL', 'phone': 'Please put a correct phone number', 'number': 'Please put a correct number', 'date': 'Please put a correct date', 'time': 'Please put a correct time (HH:mm)', 'name': 'Please put a name', 'namerus': 'Please put a correct name (only cyrillic letters)', 'nameeng': 'Please put a correct name (only latin letters)', 'string': 'You put incorrect symbols. Only letters, numbers and punctuation symbols are allowed', 'req': 'Please fill out all required fields', 'reqfield': 'Required field', 'minlength': 'Value is too short', 'maxlength': 'Value too big', 'emptyfill': 'None of the fields are filled in' }, 'RU': { 'email': 'Укажите, пожалуйста, корректный email', 'url': 'Укажите, пожалуйста, корректный URL', 'phone': 'Укажите, пожалуйста, корректный номер телефона', 'number': 'Укажите, пожалуйста, корректный номер', 'date': 'Укажите, пожалуйста, корректную дату', 'time': 'Укажите, пожалуйста, корректное время (ЧЧ:ММ)', 'name': 'Укажите, пожалуйста, имя', 'namerus': 'Укажите, пожалуйста, имя (только кириллица)', 'nameeng': 'Укажите, пожалуйста, имя (только латиница)', 'string': 'Вы написали некорректные символы. Разрешены только буквы, числа и знаки пунктуации', 'req': 'Пожалуйста, заполните все обязательные поля', 'minlength': 'Слишком короткое значение', 'maxlength': 'Слишком длинное', 'reqfield': 'Обязательное поле', 'emptyfill': 'Ни одно поле не заполнено' } } }; $(document).ready(function () {
        ...
        window.tildaForm.validate = function ($jform) {
            var arError = []; var isEmptyValue = !0; $jform.find('.js-tilda-rule').each(function () {
                var req = $(this).data('tilda-req') || 0; var rule = $(this).data('tilda-rule') || 'none', regExp, str, domainpart; var minlength = $(this).data('tilda-rule-minlength') || 0; var maxlength = $(this).data('tilda-rule-maxlength') || 0; var error = {}; var val = $(this).val(); var valnospace = ''; error.obj = $(this); error.type = []; if (val && val.length > 0) {
            ...
            $jform.find('.js-tilda-payment').remove(); var product, i, j, html = '', proddiscount = 0; window.tildaForm.tildapayment = {}; window.tildaForm.arProductsForStat = []
            ...
            window.tildaForm.tildapayment.products = []; for (i = 0; i < iProductsCount; i = i + 1) {
                product = window.tcart.products[i]; arProduct = {}; optionlabel = ''; window.tildaForm.tildapayment.products[i] = {}; for (j in product) {
                    if (typeof (product[j]) != 'function') {
                        if (j == 'options') {
                            window.tildaForm.tildapayment.products[i][j] = {}; for (var o in product[j]) {
            ...
            if (window.tcart.delivery && window.tcart.delivery.name) {
                window.tildaForm.tildapayment.delivery = { name: window.tcart.delivery.name }; if (window.tcart.delivery && window.tcart.delivery.price) {
                    pricedelivery = window.tcart.delivery.price; window.tildaForm.tildapayment.delivery.price = window.tcart.delivery.price; if (window.tcart.prodamount > 0 && typeof window.tcart.delivery.freedl != 'undefined' && window.tcart.delivery.freedl > 0) { window.tildaForm.tildapayment.delivery.freedl = window.tcart.delivery.freedl; if ((window.tcart.prodamount - proddiscount) >= window.tcart.delivery.freedl) { pricedelivery = 0 } }
        ...
        window.tildaForm.payment = function ($jform, arNext) {
            var html = ''; if ($jform.data('formpaymentoff') == 'y') { tildaForm.clearTCart($jform); return }
            ...
            $jform.addClass('js-send-form-success'); if (arNext.type == 'link') {
                tildaForm.clearTCart($jform); if (arNext.message && arNext.message > '' && $jform.find('.js-successbox').length > 0) { $jform.find('.js-successbox').html(arNext.message); $jform.find('.js-successbox').show() }
                ...
                if (arNext.type == 'form') {
                    ...
                    if (arNext.type == 'function') {
                        var arArgs = arNext.value.args; if (arNext.value.functioncode) { tildaForm.paysystemRun(arNext.value.script, arNext.value.sysname, $jform, arNext.value.functioncode, arArgs) } else { eval(arNext.value.name + '($jform, arArgs);') }
                        return !1
                    } else { tildaForm.clearTCart($jform); if (arNext.type == 'text' && arNext.message && arNext.message > '' && $jform.find('.js-successbox').length > 0) { $jform.find('.js-successbox').html(arNext.message); $jform.find('.js-successbox').show() } }
                }
            }
            ...
            window.tildaForm.clearTCart($jform); var virtPage = '/tilda/' + $jform.attr('id') + '/payment/'; var virtTitle = 'Pay order in form ' + $jform.attr('id'); var virtPrice = arArgs.amount; var virtProduct = arArgs.description; if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') {
                var currency = $('#allrecords').data('tilda-currency') || $('.t706').data('project-currency-code'); if (!currency) { $('#allrecords').data('tilda-currency', arArgs.currency) }
            ...
            $jform.data('successmessage', ''); var successcallback = $jform.data('success-callback'); window.tildaForm.successEnd($jform, arArgs.successurl, successcallback); $jform.trigger('tildaform:aftersuccess')
            ...
            window.stripehandler.open({
                name: companyname, image: companylogo, description: arArgs.description, amount: parseInt(parseFloat(arArgs.amount) * multiple), currency: arArgs.currency, shippingAddress: arArgs.shipping == '1' ? !0 : !1, email: arArgs.email > '' ? arArgs.email : '', token: function (token, args) {
                    if (token && token.id) {
                        debugger;
                        $.ajax({
                            type: "POST", url: '../send.php', data: { projectid: arArgs.projectid, invoiceid: arArgs.invoiceid, token: token.id, email: token.email, currency: arArgs.currency, amount: parseInt(parseFloat(arArgs.amount) * multiple) }, dataType: "json", xhrFields: { withCredentials: !1 }, success: function (json) {
                                window.tildaForm.clearTCart($jform); var virtPage = '/tilda/' + $jform.attr('id') + '/payment/'; var virtTitle = 'Pay order in form ' + $jform.attr('id'); var virtPrice = arArgs.amount; var virtProduct = arArgs.description; if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') {
                                    var currency = $('#allrecords').data('tilda-currency') || $('.t706').data('project-currency-code'); if (!currency) { $('#allrecords').data('tilda-currency', arArgs.currency) }
                                    Tilda.sendEventToStatistics(virtPage, virtTitle, virtProduct, virtPrice)
                                }
                ...
                if (arArgs && arArgs.html > '') {
                    $('#allrecords').append(arArgs.html); $('.t-banktransfer .t-popup__close').off('click'); $('.t-banktransfer .t-popup__close').click(function () {
                        ...
                        $jbankform.off('submit'); $jbankform.find('.t-submit').off('click'); $jbankform.find('.t-submit').off('dblclick'); $jbankform.submit(function (e) {
                            ...
                            $.ajax({
                                type: "POST", url: '../send.php', data: $jbankform.serialize(), dataType: "json", xhrFields: { withCredentials: !1 }, success: function (json) {
            ...
            var $popup = $('#tildaformsuccesspopup'); if ($popup.length > 0) {
                $('body').removeClass('t-body_success-popup-showed'); if (/iPhone|iPad|iPod/i.test(navigator.userAgent) && !window.MSStream) { window.tildaForm.unlockBodyScroll() }
                $popup.fadeOut('fast')
            }
        };
        
        window.tildaForm.lockBodyScroll = function () { var body = $("body"); if (!body.hasClass('t-body_scroll-locked')) { var bodyScrollTop = (typeof window.pageYOffset !== 'undefined') ? window.pageYOffset : (document.documentElement || document.body.parentNode || document.body).scrollTop; body.addClass('t-body_scroll-locked'); body.css("top", "-" + bodyScrollTop + "px"); body.attr("data-popup-scrolltop", bodyScrollTop) } }; window.tildaForm.unlockBodyScroll = function () {
        ...
        window.tildaForm.successEnd = function ($jform, successurl, successcallback) {
            ...
            tildaForm.clearTCart($jform); $jform.find('input[type=text]:visible').val(''); $jform.find('textarea:visible').html(''); $jform.find('textarea:visible').val(''); $jform.data('tildaformresult', { tranid: "0", orderid: "0" })
        }; 
        
        window.tildaForm.send = function ($jform, btnformsubmit, formtype, formskey) {
            window.tildaForm.tildapayment = !1; if ($jform.data('formcart') == 'y') { window.tildaForm.addPaymentInfoToForm($jform) }
            if (formtype == 2 || (!formtype && formskey > '')) {
                ...
                var theme = $('.t706__product-title').text();
                $.ajax({
                    type: "POST", url: $formurl, data: {d: d, theme: theme }, dataType: "json", success: function (json) {
                        ...
                        if (json && json.error) {
                            ...
                            $allError.html(json.error).show(); $errBox.show(); 
        ...
        window.tildaForm_initMasks = function () {
            $('.js-tilda-mask').each(function () {
                var mask = $(this).data('tilda-mask'); var maskplaceholder = $(this).data('tilda-mask-holder'); if (mask && !$(this).data('tilda-mask-init')) {
